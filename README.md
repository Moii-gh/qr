...
- `CAMERA` - сканирование QR-кодов с камеры.
- `VIBRATE` - короткая вибрация при успешном распознавании.
- `INTERNET` - зарезервировано в манифесте для сетевых сценариев.

## Перед публикацией

- Не коммитьте `.env`, `local.properties`, `*.jks`, `*.keystore`, сертификаты и ZIP-файлы подписи.
- Проверьте, что реальные пароли подписи не находятся в публичных файлах проекта.
- Очистите локальные build-артефакты (`build/`, `app/build/`) перед созданием репозитория.
- Уточните `rootProject.name`, если нужно публичное имя проекта вместо технического.

## Лицензия

Лицензия в репозитории не указана. Перед публикацией добавьте `LICENSE`, если проект должен быть доступен для использования другими разработчиками.
